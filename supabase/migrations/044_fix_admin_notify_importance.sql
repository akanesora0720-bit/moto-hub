-- Fix: message_importance is ('normal','important','urgent') — not 'high'
-- Re-apply admin notify helpers (043 fails without this)
...
begin
  ...
  if tg_op = 'UPDATE'
    ...
    perform public.notify_all_admins(
      ...
      'important',
      format('/deals/%s#deal-primary-action', new.id),
      'deals',
      new.id
    );
  end if;
