-- Fix buyer_report_payment_sent: enum + notifications must not roll back the report
...
begin
  ...
  if v.status <> 'awaiting_payment' then
    raise exception 'deal is not awaiting payment (status=%)', v.status;
  end if;
  ...
  exception
    when others then
      raise notice 'buyer_report_payment_sent deal_alert skipped: %', sqlerrm;
  end;
  ...
  exception
    when others then
      raise notice 'buyer_report_payment_sent notify_enqueue skipped: %', sqlerrm;
  end;
  ...
  exception
    when others then
      raise notice 'buyer_report_payment_sent seller email skipped: %', sqlerrm;
  end;
  ...
  exception
    when others then
      raise notice 'buyer_report_payment_sent seller notify skipped: %', sqlerrm;
  end;
  ...
  exception
    when others then
      raise notice 'buyer_report_payment_sent buyer notify skipped: %', sqlerrm;
  end;
  ...
  begin
    ...
    loop
      perform public.insert_user_notification(
        ...
        'high',
        format('/admin/workspace?tab=deals'),
        'deals',
        p_deal_id
      );
    end loop;
  exception
    when others then
      raise notice 'buyer_report_payment_sent admin notify skipped: %', sqlerrm;
  end;
...
-- Backfill alerts (safe after enum)
insert into public.deal_alerts (deal_id, alert_type, message)
select
  d.id,
  'buyer_payment_reported',
...
where d.status = 'awaiting_payment'
  ...
  and not exists (
    ...
    where a.deal_id = d.id
      and a.alert_type = 'buyer_payment_reported'
