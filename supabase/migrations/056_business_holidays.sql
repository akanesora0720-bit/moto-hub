...
comment on table public.business_holidays is '営業日計算用祝日カレンダー（JST日付）';

-- 2025–2027 国民の祝日（振替休日含む）
...
as $$
  select extract(isodow from p_day)::int between 1 and 5
    and not exists (
      select 1 from public.business_holidays h where h.holiday_date = p_day
    );
$$;
