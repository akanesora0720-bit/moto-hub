...
-- 書類・引渡遅延: 引取予定日翌営業日から handover 未完了の売り手に -5/営業日
create table if not exists public.handover_deadline_penalty_applied (
  id uuid primary key default gen_random_uuid(),
  deal_id uuid not null references public.deals (id) on delete cascade,
  penalty_date date not null,
  penalty_points int not null default 5,
...
alter table public.handover_deadline_penalty_applied enable row level security;

drop policy if exists handover_deadline_penalty_admin on public.handover_deadline_penalty_applied;
create policy handover_deadline_penalty_admin on public.handover_deadline_penalty_applied
  for all to authenticated
  using (public.is_admin())
  with check (public.is_admin());
...
begin
  select exists (
    select 1 from public.handover_deadline_penalty_applied
    where deal_id = p_deal_id and penalty_date = p_penalty_date and not waived
  ) into v_exists;
  if v_exists then return false; end if;
  ...
  insert into public.handover_deadline_penalty_applied (
    deal_id, penalty_date, penalty_points, penalty_history_id
  )
  values (p_deal_id, p_penalty_date, 5, v_hist);

  perform public.notify_enqueue(
    'handover.deadline_penalty',
    jsonb_build_object('body', format('取引 %s\n%s', p_deal_id, p_reason)),
    'deals', p_deal_id
  );
  return true;
...
begin
  for r in
    select d.id, d.seller_id, d.pickup_scheduled_at, l.maker, l.model
    from public.deals d
    join public.listings l on l.id = d.listing_id
    where d.status = 'funded'
      and d.pickup_scheduled_at is not null
      and d.handover_at is null
  loop
    if public.count_overdue_business_days(r.pickup_scheduled_at, v_today) <= 0 then
      continue;
    end if;

    v_day := ((r.pickup_scheduled_at at time zone 'Asia/Tokyo')::date) + 1;
    while v_day <= v_today loop
      if public.is_business_day(v_day) then
        v_reason := format(
          '書類・引渡期限超過（%s %s）%s',
          r.maker, r.model, v_day
        );
        if public.apply_handover_deadline_penalty(r.id, v_day, v_reason) then
          v_applied := v_applied + 1;
        end if;
      end if;
      v_day := v_day + 1;
...
grant execute on function public.run_handover_deadline_compliance_job() to authenticated;
