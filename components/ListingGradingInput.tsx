    ...
    <div className="space-y-4 rounded-xl border border-accent/25 bg-zinc-950/50 p-5">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-xs text-muted">各項目 1〜10点（オークション基準）</p>
        </div>
        ...
        {GRADING_ITEMS.map((item) => (
          <label key={item.key} className="block text-center">
            ...
            <select
              ...
              className="mt-1 w-full rounded-lg border border-border bg-card px-1 py-2.5 text-center font-serif text-lg font-semibold text-accent focus:border-accent"
              required
            >
              <option value="">—</option>
              {GRADE_OPTIONS.map((n) => (
