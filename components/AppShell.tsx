...
export function AppShell({
  ...
  const [showAdmin, setShowAdmin] = useState(!!isAdminProp);
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: auth }) => {
      ...
      supabase
        .from("profiles")
        .select("member_type, is_admin, is_active, profile_completed, contact_name, phone")
        .eq("id", auth.user.id)
        .maybeSingle()
        .then(({ data }) => {
        if (!data) return;
        const p = data as Pick<
          Profile,
          "member_type" | "is_admin" | "is_active" | "profile_completed" | "contact_name" | "phone"
        >;
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-20 border-b border-border bg-background/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3">
          <Link href={homeHref} className="text-lg font-semibold tracking-wide text-accent">
            MotoHub
          </Link>
          <nav className="flex flex-wrap items-center gap-1 text-sm">
            ...
            {!isStaff ? (
              <Link href="/onboarding" className="rounded-lg px-3 py-1.5 text-muted hover:text-foreground">
                店舗情報
              </Link>
            ) : (
              <Link href="/onboarding" className="rounded-lg px-3 py-1.5 text-muted hover:text-foreground">
