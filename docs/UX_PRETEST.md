...
Moto-Hub プレテストで発見した UX・取引フロー改善の仕様メモです。
...
- 成約確定（`admin_finalize_agreement`）時に `ensure_deal_billing` で買い手向け入金指示書を作成し、**即時送信**（`admin_approve_and_send_invoices`）
- `system_settings.billing.auto_send_invoices` は `true`
- レガシー案件で `review_pending` が残る場合のみ `/admin/billing` から一括承認可能

## Moto-Hub手数料（週次請求）

- 取引ごとの即時手数料請求は廃止（`platform_fee` / `part_platform_fee` の下書きは移行時に取消）
- 車両：引取完了（`pickup_completed_at`）で `platform_fee_accruals` に計上
- パーツ：発送完了または引渡完了で計上
- 集計週：土曜0:00〜金曜23:59（JST）。毎週月曜に `weekly_vehicle_platform_fee` / `weekly_part_platform_fee` を別発行
- 支払期限：発行日を含む3営業日。加盟店は `/my/payments` でPDF確認
